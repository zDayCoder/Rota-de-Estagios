    ...
    <div class="container">
        ...
        <form action="{{ route('vacancy.update', $vacancy->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="addreess_id" name="addreess_id" value="{{ $vacancy->addreess_id }}" required>
            </div>
            ...
            <div class="form-group">
                ...
                <div id="skills">
                    @foreach ($skills as $index => $skill)
                        <div class="skill row">
                            <div class="col-md-4">
                                <input type="text" class="form-control" name="skills[{{ $index }}][name]" placeholder="Skill Name" value="{{ $skill->name }}" required>
                            </div>
                            <div class="col-md-4">
                                <input type="text" class="form-control" name="skills[{{ $index }}][level]" placeholder="Skill Level" value="{{ $skill->level }}" required>
                            </div>
                            <div class="col-md-4">
                                <input type="text" class="form-control" name="skills[{{ $index }}][curriculum_id]" placeholder="Curriculum ID" value="{{ $skill->vacancy_id }}" required>
                            </div>
                        </div>
                    @endforeach
                </div>
